  ...
  <AppLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      <BaseDialog :show="showEditor" :title="editingId ? t('common.edit') : t('admin.scheduledJobs.create')" width="wide" @close="closeEditor">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            ...
            <template v-else-if="form.job_type === 'track_relay_codex_group_ratio'">
              <div class="space-y-4">
                <div>
                  <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.scheduledJobs.accountNameContains') }}</label>
                  <input v-model="relayCodexRatioForm.account_name_contains" class="input w-full" placeholder="云锦" />
                  <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.scheduledJobs.accountNameContainsHint') }}</p>
                </div>
                <div class="space-y-3">
                  <div class="flex items-center justify-between gap-3">
                    <div>
                      <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.scheduledJobs.ratioMappingRules') }}</label>
                      <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.scheduledJobs.ratioMappingRulesHint') }}</p>
                    </div>
                    <button type="button" class="btn btn-secondary btn-xs" @click="addRelayCodexRatioRule">
                      {{ t('admin.scheduledJobs.addRatioRule') }}
                    </button>
                  </div>
                  <div class="space-y-2">
                    <div
                      v-for="(rule, index) in relayCodexRatioRules"
                      :key="rule.id"
                      class="grid grid-cols-1 gap-2 rounded-xl border border-gray-200 p-3 dark:border-dark-600 md:grid-cols-[160px_1fr_auto]"
                    >
                      <div>
                        <label class="mb-1 block text-[11px] font-medium text-gray-500 dark:text-gray-400">{{ t('admin.scheduledJobs.ratio') }}</label>
                        <input v-model.number="rule.ratio" type="number" step="0.0001" min="0" class="input w-full" />
                      </div>
                      <div>
                        <label class="mb-1 block text-[11px] font-medium text-gray-500 dark:text-gray-400">{{ t('admin.scheduledJobs.targetModel') }}</label>
                        <Select v-model="rule.target_model" :options="openAIModelCandidateOptions" />
                      </div>
                      <div class="flex items-end">
                        <button type="button" class="btn btn-danger btn-xs w-full md:w-auto" @click="removeRelayCodexRatioRule(index)">
                          {{ t('common.delete') }}
                        </button>
                      </div>
                    </div>
      ...
      <BaseDialog :show="showLogs" :title="t('admin.scheduledJobs.logs')" width="wide" @close="closeLogs">
        <div class="space-y-3">
          <div v-if="currentLogsJobIsRelayRatio" class="flex justify-end">
            <button type="button" class="btn btn-secondary btn-sm" :disabled="ratioTrendLoading" @click="openRatioTrend">
              {{ ratioTrendLoading ? t('common.loading') : t('admin.scheduledJobs.viewRatioTrend') }}
            </button>
          </div>
      ...
      <BaseDialog :show="showRatioTrend" :title="t('admin.scheduledJobs.ratioTrendTitle')" width="wide" @close="closeRatioTrend">
        <div class="space-y-4">
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div class="rounded-xl bg-gray-50 px-4 py-3 dark:bg-dark-700/70">
              <div class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.scheduledJobs.ratioTrendGroup') }}</div>
              <div class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">codex</div>
            </div>
            <div class="rounded-xl bg-gray-50 px-4 py-3 dark:bg-dark-700/70">
              <div class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.scheduledJobs.ratioTrendWindow') }}</div>
              <div class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.scheduledJobs.lastThreeDays') }}</div>
            </div>
            <div class="rounded-xl bg-gray-50 px-4 py-3 dark:bg-dark-700/70">
              <div class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.scheduledJobs.ratioTrendLatest') }}</div>
              <div class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ latestRatioLabel }}</div>
            </div>
          </div>
          <div class="h-[360px] rounded-xl border border-gray-200 p-3 dark:border-dark-700">
            <Line v-if="ratioTrendChartData" :data="ratioTrendChartData" :options="ratioTrendChartOptions" />
            <div v-else class="flex h-full items-center justify-center text-sm text-gray-500 dark:text-gray-400">
              {{ ratioTrendLoading ? t('common.loading') : t('common.noData') }}
            </div>
          </div>
        </div>
      </BaseDialog>
...
import type { AdminGroup, AdminScheduledJob, AdminScheduledJobRun, AdminScheduledOpenAIOAuthModelMappingPayload, AdminScheduledRelayCodexGroupRatioPayload, AdminScheduledSyncCodexFreeGroupsPayload, CreateAdminScheduledJobRequest, UpdateAdminScheduledJobRequest } from '@/types'
...
const ratioTrendRuns = ref<AdminScheduledJobRun[]>([])
const showEditor = ref(false)
const showLogs = ref(false)
const showRatioTrend = ref(false)
const ratioTrendLoading = ref(false)
...
const jobTypeOptions = [
  ...
  { value: 'track_relay_codex_group_ratio', labelKey: 'admin.scheduledJobs.types.track_relay_codex_group_ratio' },
] as const
...
const relayCodexRatioForm = reactive({
  account_name_contains: '云锦',
})
const openAIModelMappingRows = ref<OpenAIModelMappingRow[]>([])
const relayCodexRatioRules = ref<RelayCodexRatioRuleRow[]>([])
let openAIModelMappingRowID = 1
let relayCodexRatioRuleID = 1
...
const currentLogsJob = computed(() => jobs.value.find((job) => job.id === currentLogsJobId.value) || null)
const currentLogsJobIsRelayRatio = computed(() => currentLogsJob.value?.job_type === 'track_relay_codex_group_ratio')
...
watch(
  ...
  (jobType) => {
    ...
    form.name = formatJobType(jobType)
    form.cron_expression = defaultCronExpression(jobType)
    form.retention_limit = defaultRetentionLimit(jobType)
  }
)

function defaultCronExpression(jobType: string) {
  if (jobType === 'track_relay_codex_group_ratio') return '*/10 * * * *'
  return '0 * * * *'
}

function defaultRetentionLimit(jobType: string) {
  if (jobType === 'track_relay_codex_group_ratio') return 500
  return 100
}
...
function resetForm() {
  ...
  form.cron_expression = defaultCronExpression(form.job_type)
  form.enabled = true
  form.payload_json = '{}'
  form.retention_limit = defaultRetentionLimit(form.job_type)
...
function openEdit(job: AdminScheduledJob) {
  ...
  if (job.job_type === 'sync_codex_free_group_accounts') {
    ...
    resetRelayCodexRatioForm()
  } else if (job.job_type === 'track_relay_codex_group_ratio') {
    syncCodexFreeForm.source_group_id = 0
    syncCodexFreeForm.target_group_ids = []
    openAIModelMappingRows.value = defaultOpenAIModelMappingRows()
    parseRelayCodexRatioPayload(job.payload_json)
  } else {
    syncCodexFreeForm.source_group_id = 0
    syncCodexFreeForm.target_group_ids = []
    openAIModelMappingRows.value = defaultOpenAIModelMappingRows()
...
function closeLogs() {
  ...
  showRatioTrend.value = false
  currentLogsJobId.value = null
  runs.value = []
  ratioTrendRuns.value = []
}

type RelayRatioTrendPoint = {
  ratio: number
  fetched_at: string
}

async function openRatioTrend() {
  if (!currentLogsJobId.value) return
  showRatioTrend.value = true
  ratioTrendLoading.value = true
  try {
    ratioTrendRuns.value = await adminAPI.scheduledJobs.listRuns(currentLogsJobId.value, 500)
  } finally {
    ratioTrendLoading.value = false
  }
}
...
function parseRelayRatioRun(run: AdminScheduledJobRun): RelayRatioTrendPoint | null {
  if (run.status !== 'success' || !run.result_json || run.result_json === '{}') return null
  try {
    const payload = JSON.parse(run.result_json) as { codex_ratio?: unknown, fetched_at?: unknown }
    const ratio = Number(payload.codex_ratio)
    const fetchedAt = typeof payload.fetched_at === 'string' ? payload.fetched_at : run.created_at
    if (!Number.isFinite(ratio)) return null
    return { ratio, fetched_at: fetchedAt }
  } catch {
    return null
  }
}

const ratioTrendPoints = computed(() => {
  const cutoff = Date.now() - 3 * 24 * 60 * 60 * 1000
  return ratioTrendRuns.value
    .map(parseRelayRatioRun)
    .filter((point): point is RelayRatioTrendPoint => {
      if (!point) return false
      const time = Date.parse(point.fetched_at)
      return Number.isFinite(time) && time >= cutoff
    })
    .sort((a, b) => Date.parse(a.fetched_at) - Date.parse(b.fetched_at))
})

const latestRatioLabel = computed(() => {
  const latest = ratioTrendPoints.value[ratioTrendPoints.value.length - 1]
  return latest ? latest.ratio.toFixed(4) : '-'
})

function formatRatioTrendLabel(value: string) {
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) return value
  return `${String(date.getMonth() + 1).padStart(2, '0')}/${String(date.getDate()).padStart(2, '0')} ${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`
}

const ratioTrendChartData = computed(() => {
  if (!ratioTrendPoints.value.length) return null
  return {
    labels: ratioTrendPoints.value.map((point) => formatRatioTrendLabel(point.fetched_at)),
    datasets: [
      {
        label: t('admin.scheduledJobs.codexRatio'),
        data: ratioTrendPoints.value.map((point) => point.ratio),
        borderColor: '#0f766e',
...
const ratioTrendChartOptions = computed(() => ({
  responsive: true,
    ...
    tooltip: {
      callbacks: {
        label: (context: { parsed?: { y?: number | null } }) => {
          ...
          return `${t('admin.scheduledJobs.codexRatio')}: ${value.toFixed(4)}`
        }
      }
    }
  },
...
function createRelayCodexRatioRuleRow(ratio = 0.06, targetModel = 'gpt-5.4-mini'): RelayCodexRatioRuleRow {
  return { id: relayCodexRatioRuleID++, ratio, target_model: targetModel }
}

function defaultRelayCodexRatioRules() {
  return [
    createRelayCodexRatioRuleRow(0.06, 'gpt-5.4-mini'),
    createRelayCodexRatioRuleRow(0.07, 'gpt-5.4'),
    createRelayCodexRatioRuleRow(0.08, 'gpt-5.5'),
  ]
}

function resetRelayCodexRatioForm() {
  relayCodexRatioForm.account_name_contains = '云锦'
  relayCodexRatioRules.value = defaultRelayCodexRatioRules()
}

function parseRelayCodexRatioPayload(payloadJSON: string) {
  try {
    const payload = JSON.parse(payloadJSON || '{}') as Partial<AdminScheduledRelayCodexGroupRatioPayload>
    relayCodexRatioForm.account_name_contains = String(payload.account_name_contains || '云锦')
    const rules = Array.isArray(payload.ratio_mapping_rules) ? payload.ratio_mapping_rules : []
    relayCodexRatioRules.value = rules.length
      ? rules.map((rule) => createRelayCodexRatioRuleRow(Number(rule.ratio || 0), String(rule.target_model || Object.values(rule.model_mapping || {})[0] || '')))
      : defaultRelayCodexRatioRules()
  } catch {
    resetRelayCodexRatioForm()
  }
}

function addRelayCodexRatioRule() {
  relayCodexRatioRules.value = [...relayCodexRatioRules.value, createRelayCodexRatioRuleRow()]
}

function removeRelayCodexRatioRule(index: number) {
  relayCodexRatioRules.value = relayCodexRatioRules.value.filter((_, i) => i !== index)
}
...
function buildRelayCodexRatioPayload(): AdminScheduledRelayCodexGroupRatioPayload {
  return {
    account_name_contains: relayCodexRatioForm.account_name_contains.trim(),
    ratio_mapping_rules: relayCodexRatioRules.value
      .map((rule) => ({
        ratio: Number(rule.ratio),
        target_model: rule.target_model.trim(),
      }))
      .filter((rule) => Number.isFinite(rule.ratio) && rule.ratio > 0 && rule.target_model),
  }
}
...
async function submitForm() {
  ...
  try {
    ...
    } else if (form.job_type === 'track_relay_codex_group_ratio') {
      const relayPayload = buildRelayCodexRatioPayload()
      if (!relayPayload.account_name_contains) {
        appStore.showError(t('admin.scheduledJobs.accountNameContainsRequired'))
        return
      }
      if (relayPayload.ratio_mapping_rules.length === 0) {
        appStore.showError(t('admin.scheduledJobs.ratioRulesRequired'))
        return
      }
    }
    const payloadJSON = form.job_type === 'sync_codex_free_group_accounts'
      ...
      : form.job_type === 'track_relay_codex_group_ratio'
        ? JSON.stringify(buildRelayCodexRatioPayload())
...
function formatJobMessage(message: string) {
  ...
  const relayRatioMatch = message.match(/^relay\s+codex\s+group\s+ratio\s+([0-9.]+)$/i)
  if (relayRatioMatch) {
    return t('admin.scheduledJobs.relayRatioResult', { ratio: relayRatioMatch[1] })
  }
  const relayRatioMappingMatch = message.match(/^relay\s+codex\s+group\s+ratio\s+([0-9.]+);\s+updated\s+(\d+)\s+yunjin\s+openai oauth accounts$/i)
  if (relayRatioMappingMatch) {
    return t('admin.scheduledJobs.relayRatioMappingResult', {
      ratio: relayRatioMappingMatch[1],
      accounts: relayRatioMappingMatch[2],
    })
  }
  const relayRatioNoRuleMatch = message.match(/^relay\s+codex\s+group\s+ratio\s+([0-9.]+);\s+no\s+mapping\s+rule\s+matched$/i)
  if (relayRatioNoRuleMatch) {
    return t('admin.scheduledJobs.relayRatioNoRuleResult', { ratio: relayRatioNoRuleMatch[1] })
  }
  return message
}
