export default {
    accounts: {
      ...
      poolUpstream: {
        platform: '池上游平台',
        platformHint: '声明上游服务类型以启用信息探测；仅池模式 API Key 账号可用，关闭池模式会保存为默认。',
        platformDefault: '默认（不探测）',
        platformSub2api: 'Sub2API',
        platformChatgpt2api: 'ChatGPT2API（仅 OpenAI）',
        features: '探测内容',
        featuresHint: '选择需要定期获取的上游信息。',
        feature_balance: '余额（GET /v1/usage，USD）',
        feature_account_count: '正常账号数（accounts.active）',
        feature_image_quota: '已知绘图额度（total_quota）',
        manualRefresh: '立即刷新上游信息',
        notProbed: '未探测',
        stale: '已过期',
        unsupported: '不支持',
        failed: '失败',
        amount_wallet: '钱包余额：{value}',
        amount_quota: '剩余额度：{value}',
        amount_subscription: '订阅剩余：{value}',
        accountsActive: '正常账号：{value}',
        totalQuota: '已知绘图额度：{value}',
        unlimitedQuota: '不限额度账号：{value}',
        unknownQuota: '额度未知账号：{value}',
        updatedAt: '更新时间：{value}',
        nextProbeAt: '下一次探测：{value}',
        probeFailed: '刷新池上游信息失败',
        errors: {
          request_failed: '请求失败',
          empty_response: '响应为空',
          response_read_failed: '读取响应失败',
          response_too_large: '响应超过大小限制',
          invalid_response: '无法识别响应格式',
          invalid_base_url: '上游地址无效',
          missing_api_key: '缺少 API Key',
          proxy_unavailable: '代理不可用',
          transport_unavailable: '探测传输不可用',
          request_build_failed: '请求构建失败',
          unauthorized: '上游鉴权失败（401）',
          forbidden: '上游拒绝访问（403）',
          http_error: '上游返回错误',
          unsupported: '上游不支持该探测',
          probe_failed: '探测失败'
        }
      },
