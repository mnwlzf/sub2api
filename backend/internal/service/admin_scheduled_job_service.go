...
func (s *AdminScheduledJobService) Create(ctx context.Context, p AdminScheduledJobCreateParams) (*AdminScheduledJob, error) {
	retentionLimit := normalizeAdminScheduledJobRetention(p.JobType, p.RetentionLimit)
	if err := validateAdminScheduledJob(p.JobType, p.CronExpression, p.PayloadJSON, retentionLimit); err != nil {
		return nil, err
	}
...
func (s *AdminScheduledJobService) Update(ctx context.Context, id int64, p AdminScheduledJobUpdateParams) (*AdminScheduledJob, error) {
	...
	if p.RetentionLimit != nil {
		job.RetentionLimit = normalizeAdminScheduledJobRetention(job.JobType, *p.RetentionLimit)
	}
...
func validateAdminScheduledJob(jobType, cronExpr, payloadJSON string, retentionLimit int) error {
	...
	if strings.TrimSpace(jobType) == AdminScheduledJobTypeTrackRelayCodexGroupRatio && retentionLimit < 432 {
		return fmt.Errorf("retention_limit must be at least 432 for 3 days of 10-minute samples")
	}
...
func validateAdminScheduledJobPayload(jobType, payloadJSON string) error {
	...
	case AdminScheduledJobTypeTrackRelayCodexGroupRatio:
		return validateAdminScheduledRelayCodexGroupRatioPayload(payloadJSON)
	}
	return nil
}

func validateAdminScheduledRelayCodexGroupRatioPayload(payloadJSON string) error {
	var payload adminScheduledRelayCodexGroupRatioPayload
	if err := json.Unmarshal([]byte(payloadJSON), &payload); err != nil {
		return fmt.Errorf("invalid payload_json: %w", err)
	}
	if strings.TrimSpace(payload.AccountNameContains) == "" && len(payload.RatioMappingRules) > 0 {
		return fmt.Errorf("account_name_contains is required")
	}
	for _, rule := range payload.RatioMappingRules {
		if rule.Ratio <= 0 {
			return fmt.Errorf("ratio_mapping_rules contains invalid ratio")
		}
		mapping := relayCodexRuleModelMapping(rule)
		if len(mapping) == 0 {
			return fmt.Errorf("ratio_mapping_rules contains empty model mapping")
		}
		for source, target := range mapping {
			if strings.TrimSpace(source) == "" || strings.TrimSpace(target) == "" {
				return fmt.Errorf("ratio_mapping_rules contains empty model")
			}
		}
	}
	return nil
...
func normalizeAdminScheduledJobRetention(jobType string, v int) int {
	if v <= 0 {
		if strings.TrimSpace(jobType) == AdminScheduledJobTypeTrackRelayCodexGroupRatio {
			return 500
		}
		return 100
	}
